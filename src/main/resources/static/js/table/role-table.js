	        	...
	        	$('.ccc').each(function(i){
	        		...
	        		$(this).on('click',function(e){
					    ...
					    $('#record_id').val(recordid);
						  $.ajax({
	        				   url:$('body').attr('backend_url')+'/management/roles/one-role?id='+recordid,
	        				   method:'get',
	        				   async:false,
	        				   contentType: "application/json",
	        				   success:function(e){
									$("#nameupdate").val(e.name);
	        						$("#descriptionupdate").val(e.description);
	        						for(var i=0; i < e.functions.length; i++){
	        							 console.log(e.functions[i].functionsGroupName)
	        							 {
	        								 $(".functionsGroupName3."+e.functions[i].functionsGroupName).attr('checked','checked');
	        							 }
	        						}
	        		...
	        		$(this).on('click',function(){
	        			...
	        			$("#recordid").val(recordid);
	        			  ...
	        			  $.ajax({
	        				   ...
	        				   success:function(e){
									$("#nameview").val(e.name);
	        						$("#descriptionview").val(e.description);
	        						...
	        						for(var i=0; i < e.functions.length; i++){
	        							console.log(e.functions[i].functionsGroupName)
	        								 ...
	        								 $(".functionsGroupName2."+e.functions[i].functionsGroupName).prop('checked','checked');
