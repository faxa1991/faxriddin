	...
	var datatableInit = function() {
		...
		$table.dataTable({
		  ...
		  aoColumns: [
	            ...
	            { "mData": "asserMovePTime","defaultContent":""},
	            { "mData": "assetHoldPTime","defaultContent":""}
	        	...
	        	$('.ccc').each(function(i){
	        		...
	        		$(this).on('click',function(e){
					    ...
					    $('#employe_id').val(recordid);
		        		   $.ajax({
		        			   url: $('body').attr('backend_url')+"/api/reports/v1/planassetmovement/one?id="+recordid,
		        			   method:"POST",
		        			   contentType: "application/json",
		        			   data:recordid,
		        			   success:function(data){
			                        $("#equipment_2").val(data.equipment.id);
			                        $("#equipment_2").trigger('change');
			                        $("#asserMovePTime_2").val(data.asserMovePTime);
			  					    $("#assetHoldPTime_2").val(data.assetHoldPTime);
