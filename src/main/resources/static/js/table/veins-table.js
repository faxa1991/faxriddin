	        		...
	        		$(this).on('click',function(e){
	        			var recordid = $(this).attr('recordid');
				            $("#recordid").val(recordid);
	        				  $.ajax({
	        				   url:$('body').attr('backend_url')+'/api/reports/v1/lode/one?id='+recordid,
	        				   method:'POSt',
	        				   async:false,
	        				   contentType: "application/json",
	        				   success:function(e){
	        					    $("#minefields2").val(e.minefields.id);
	        					    $("#minefields2").trigger('change');
	        					    $("#veins2").val(e.name)
	        				   } 
	    ...
	    $('.deleteRecord').each(function(i){
	        		...
	        		$(this).on('click',function(e){
	        			...
	        			deleteRecord("Вы действительно хотите удалить запись?",function(){
	        				    $.ajax({
	        				    	url:$('body').attr('backend_url')+'/api/reports/v1/lode/delete',
	        				    	method:'POST',
	        				    	contentType: "application/json",
	        				    	data:recordid,
	        				    	success:function(t){
