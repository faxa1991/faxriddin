					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Сотрудники</span></li>
								<li><span>Редактирование сотрудника</span></li>
							</ol>
							...
							<div class="col-xs-12">
								<section class="panel">
									<header class="panel-heading">
										...
										<h2 class="panel-title">Редактирование сотрудника</h2>
									</header>
		...
		$.ajax({
				...
				success:function(data){
					...
					for(var i=0;i<data.data.length;i++){
						...
						/* $("#mploye-type-select").val(data.data[0].id);
						$("#mploye-type-select").trigger('change'); */
			...
			$.ajax({
				url:"${backend_url}/api/reports/v1/employe/one",
				method:"POST",
				contentType: "application/json",
				data: "${employe_id}",
				async:false,
				success:function(data){
					...
					$("#tabelNumber").val(data.data.tabelNumber);
					
					if(data.data.employeType!=null){
						$("#employe-type-select").val(data.data.employeType.id);
						$("#employe-type-select").trigger('change');
					}
					
					if(data.data.shift!=null){
						$("#shift").val(data.data.shift.id);
						$("#shift").trigger('change');
					}
				}
			...
			attachValidator("editform",function(){
				...
				var employe = stor.one;
				
				 delete stor.one["fio"];
				 stor.one.firstName = $('#firstName').val();
				 stor.one.lastName = $('#lastName').val();
				 stor.one.tabelNumber = $('#tabelNumber').val();
				 stor.one.employeType = {"id":$('#employe-type-select').val()};
				 
				 if($('#shift').val()!=undefined && $('#shift').val()=='0')
				 	stor.one.shift =null;
				 else
					 stor.one.shift ={ "id":$('#shift').val()};
				...
				$.ajax({
					url:"${backend_url}/api/reports/v1/employe/update",
					method:"POST",
					contentType: "application/json",
