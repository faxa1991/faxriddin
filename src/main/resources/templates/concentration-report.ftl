...
<html class="fixed">
	<head>
		...
		<title>Отчет о рисках связанных с концентрацией газов</title>
	...
	<body backend_url="${backend_url}">
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						<h2>Отчет о рисках связанных с концентрацией газов</h2>
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Отчет о рисках связанных с концентрацией газов</span></li>
					...
					<!-- start: page -->
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Отчет о рисках связанных с концентрацией газов</h2>
							</header>
							<div class="panel-body">
								       ...
								       <form>
								        <div class="col-sm-4">
								          	<div class="form-group col-sm-12">
												 <label class="control-label">Все</label>
								          ...
								          <div class="col-sm-4">
										   	<div class="form-group col-sm-12">
												 <label class="control-label">Тип датчика</label>
												  <select id="tybdatchyka" name="tybdatchyka" data-plugin-selectTwo class="form-control populate">
								        ...
								        <div class="col-sm-4">
                                              <div class="form-group col-sm-12">
												 <label class="control-label">Номер датчика</label>
												  <select id="nomerdatchyka" name="nomerdatchyka" data-plugin-selectTwo class="form-control populate">
								...
								<table class="table table-bordered table-striped" id="datatable-ajax" data-url="${backend_url}/api/reports/v1/otamsrisk/data">
									 <thead>
										 <tr>
											<th  rowspan="2">Дата</th>
											<th  rowspan="2">Смена</th>
											<th  rowspan="2">Наименование места</th>
											<th  rowspan="2">Датчик</th>
											<th  rowspan="2">Тип датчика</th>
										    <th  rowspan="2">Ед.изм.</th>
											<th colspan="4">Максимальная концентрация газа</th>
										</tr> 
									      <tr>
											<th>П</th>
											<th>Ф</th>
		...
		<!-- Examples -->
		<script src="/js/table/concentration-report-table.js"></script>
     ...
     $.ajax({
	   ...
	   $.ajax({
			  url:'${backend_url}/api/reports/v1/list/amsnames',
			  method:'POST',
			  contentType: "application/json",
			  async:false,
			  data:JSON.stringify({"start":0,"length":1000}),
			  success:function(data){
				  $('#nomerdatchyka').find('option').remove();
				  $('#nomerdatchyka').append('<option></option>');
				  for(var i=0;i<data.length;i++){
					  $('#nomerdatchyka').append('<option value="'+data[i]+'">'+data[i]+'</option>');
				  }
			  }
	   ...
	   $.ajax({
			  url:'${backend_url}/api/reports/v1/list/amstypes',
			  method:'POST',
			  contentType: "application/json",
			  async:false,
			  data:JSON.stringify({"start":0,"length":10000}),
			  success:function(data){
				  $('#tybdatchyka').find('option').remove();
				  $('#tybdatchyka').append('<option></option>');
				  for(var i=0;i<data.length;i++){
					  $('#tybdatchyka').append('<option value="'+data[i]+'">'+data[i]+'</option>');
				  }
			  }
		   ...
		   $("#refresh").unbind('click');
		    $("#refresh").on('click',function(){
		       ...
		       $("#nomerdatchyka").select2('data', null);
		       $("#tybdatchyka").select2('data', null);
		       ...
		       $('#datatable-ajax').dataTable().fnDraw(true);
		    	...
		    	$("#load").on('click',function(){
		          $.ajax({
		            url:'${backend_url}/api/reports/v1/otamsrisk/xlsx',
		            method:'POST',
		            contentType: "application/json",
		            async:false,
		            ...
		            success:function(data){
		              console.log(data);
		              window.location.href = '${backend_url}/api/reports/v1/otamsrisk/xlsx?key='+data;
		            }
		           });  
		        })
