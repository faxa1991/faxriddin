...
<html class="fixed">
	<head>
		...
		<title>Отчет о времени выполнения цикла горных работ</title>
	...
	<body backend_url="${backend_url}">
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						<h2>Отчет о времени выполнения цикла горных работ</h2>
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Отчет о времени выполнения цикла горных работ</span></li>
					...
					<!-- start: page -->
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Отчет о времени выполнения цикла горных работ</h2>
							</header>
							<div class="panel-body">
								       ...
								       <form>
								        <div class="col-sm-4">
								          	<div class="form-group col-sm-12">
												 <label class="control-label">Забой</label>
												  <select id="zaboy" name="zaboy" data-plugin-selectTwo class="form-control populate">
															
											  </select>
										   </div>
										   	<div class="form-group col-sm-12">
												 <label class="control-label">Рудник</label>
												  <select id="rudnik" name="rudnik" data-plugin-selectTwo class="form-control populate">
								          ...
								          <div class="col-sm-4">
										   	<div class="form-group col-sm-12">
												 <label class="control-label">Место отгрузки</label>
												  <select id="place" name="place" data-plugin-selectTwo class="form-control populate">
										   ...
										   <div class="form-group col-sm-12">
												 <label class="control-label">Транспорт</label>
												  <select id="transport" name="transport" data-plugin-selectTwo class="form-control populate">
								        ...
								        <div class="col-sm-4">
                                              <div class="form-group col-sm-12">
												 <label class="control-label">Смена</label>
												  <select id="smena" name="smena" data-plugin-selectTwo class="form-control populate">
								...
								<table class="table table-bordered table-striped" id="datatable-ajax" data-url="${backend_url}/api/reports/v1/otequipmenttriptime/data">
									 <thead>
										 <tr>
											<th >Дата</th>
											<th >Смена</th>
		...
		<!-- Examples -->
		<script src="/js/table/time-report-on-mining-table.js"></script>
     ...
     $.ajax({
			  ...
			  success:function(data){
				  $('#zaboy').find('option').remove();
				  $('#zaboy').append('<option></option>');
				  
				  $('#place').find('option').remove();
				  $('#place').append('<option></option>');
				  for(var i=0;i<data.data.length;i++){
					   console.log(data.data[i].name)
					  $('#zaboy').append('<option value="'+data.data[i].id+'">'+data.data[i].name+'</option>');
					  $('#place').append('<option value="'+data.data[i].id+'">'+data.data[i].name+'</option>');
				  }
			  }
	   ...
	   $.ajax({
			  ...
			  success:function(data){
				  $('#smena').find('option').remove();
				  $('#smena').append('<option></option>');
				  for(var i=0;i<data.data.length;i++){
					  $('#smena').append('<option value="'+data.data[i].id+'">'+data.data[i].shift.name+'</option>');
				  }
			  }
	   ...
	   $.ajax({
			  ...
			  data:JSON.stringify({"start":0,"length":10000}),
			  success:function(data){
				  $('#rudnik').find('option').remove();
				  $('#rudnik').append('<option></option>');
				  for(var i=0;i<data.data.length;i++){
					  $('#rudnik').append('<option value="'+data.data[i].id+'">'+data.data[i].name+'</option>');
				  }
			  }
		...
		$.ajax({
			...
			data:JSON.stringify({"start":0,"length":10000}),
			success:function(data){
				  $('#transport').find('option').remove();
				  $('#transport').append('<option></option>');
				for(var i=0;i<data.data.length;i++){
					$('#transport').append('<option value="'+data.data[i].id+'">'+data.data[i].name+'</option>');
		   ...
		   $("#refresh").unbind('click');
		    $("#refresh").on('click',function(){
			   $("#zaboy").select2('data', null);
		       $("#place").select2('data', null);
		       $("#rudnik").select2('data', null);
		       $("#transport").select2('data', null);
		       $("#smena").select2('data', null);
		    ...
		    $("#load").on('click',function(){
		          $.ajax({
		            url:'${backend_url}/api/reports/v1/otequipmenttriptime/xlsx',
		            method:'POST',
		            contentType: "application/json",
		            async:false,
		            ...
		            success:function(data){
		              console.log(data);
		              window.location.href = '${backend_url}/api/reports/v1/otequipmenttriptime/xlsx?key='+data;
		            }
		           });  
		        })
