...
<html class="fixed">
	<head>
		...
		<title>Отчет о времени работы  транспорта</title>
	...
	<body backend_url="${backend_url}">
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						<h2>Отчет о времени работы  транспорта</h2>
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Отчет о времени работы  транспорта</span></li>
					...
					<!-- start: page -->
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Отчет о времени работы  транспорта</h2>
							</header>
							<div class="panel-body">
								       ...
								       <form>
								        <div class="col-sm-4">
								          	<div class="form-group col-sm-12">
												 <label class="control-label">Наименование обьекта</label>
								...
								<table class="table table-bordered table-striped" id="datatable-ajax" data-url="${backend_url}/api/reports/v1/otequipmenttimezone/data">
									 <thead>
										 <tr>
											<th rowspan="2">Дата</th>
											<th rowspan="2">Смена</th>
											<th rowspan="2">Наименование обьекта</th>
											<th rowspan="2">Тип обьекта</th>
											<th colspan="3">Суммарное время работы транспорта, мин</th>
											<th colspan="3">Время работы транспорта в движении, мин</th>
										    <th colspan="3">Время работы транспорта без движения, мин</th>
		...
		<!-- Examples -->
		<script src="/js/table/сountdown-time-transport-table.js"></script>
	   ...
	   $.ajax({
			  ...
			  success:function(data){
				  ...
				  for(var i=0;i<data.data.length;i++){
					  $('#typeobject').append('<option value="'+data.data[i].id+'">'+data.data[i].equipmentType.name+'</option>');
				  }
			  }
	   ...
	   $.ajax({
		...
		$.ajax({
			...
			success:function(data){
				...
				for(var i=0;i<data.data.length;i++){
					$('#objectname').append('<option value="'+data.data[i].id+'">'+data.data[i].name+'</option>');
		   ...
		   $("#refresh").unbind('click');
		    $("#refresh").on('click',function(){
		     ...
		     $("#load").on('click',function(){
		          $.ajax({
		            url:'${backend_url}/api/reports/v1/otequipmenttimezone/xlsx',
		            method:'POST',
		            contentType: "application/json",
		            async:false,
		            ...
		            success:function(data){
		              console.log(data);
		              window.location.href = '${backend_url}/api/reports/v1/otequipmenttimezone/xlsx?key='+data;
		            }
		           });  
		        })
